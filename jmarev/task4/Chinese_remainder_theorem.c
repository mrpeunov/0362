...
int ChineseRemainderTheorem(size_t N, size_t* Mxc_summ, size_t* M_main, modular_arithmetic* designation)
{
    ...
    for(i = 0; i < N; i++)
    {
        ...
        if(designation[i].remainder<0)
        {
            if(-designation[i].remainder>designation[i].module)
            {
                designation[i].remainder %= designation[i].module;
            };
            designation[i].remainder = designation[i].remainder + designation[i].module;
        };
...
int main()
{
    ...
    for(i = 0; i < N; i++)
    {
        scanf("%d",&designation[i].remainder);
        while(1)
        {
            scanf("%d",&designation[i].module);
            if(designation[i].module>0)
            {
                break;
            };
            printf("Module must have natural value, enter it again\n");
        };
        printf("\n");
    };
