...
/* Программа работает только с алфавитом в виде последовательных десятичных цифр в промежутке [1, n], где
 * n - введенная пользователем мощность алфавита.
*/
...
int nextPermByTheCurrent(int a[], int n){
    ...
    for (int i = n - 2; i >= 0; i--) {       // обходим перестановку справа, начиная с предпоследнего элемента
        if (a[i] < a[i + 1]){                  // ищем первый элемент, сосед которого справа больше
            ...
            for (int j = i + 2; j < n; j++) {    // находим такой номер j,что число a_j является наименьшим среди чисел
                                                                   // с индексами [i + 1, n], превосходящим a_i.
                if (a[i] < a[j] && a[j] < a[min]) min = j;
            }

            swap(a[i], a[min]);     // меняем местами a_i и a_j
            reverse(&a[i+1], &a[n]); // изменяем порядок элементов, занимающих места [i + 1, n],
            // на противоположный.
 ...
 * На вход принимается res[] - массив элементов перестановки, pwr - мощность алфавита,
 * указатель на функцию fromFactorialNumberSystem(int N).
 ...
 * На вход принимается num - номер перестановки, pwr - мощность алфавита,
 * указатель на функцию toFactorialNumberSystem(int N).
